...
| `.claude/task_archive.json` | Done/Cancelled 任务归档，保留 id 序列 | Agent 写 |
| `.claude/recording.md` | Session 进度记录，每次追加 | Agent 写 |
| `.claude/recording_archive/YYYY-MM.md` | 按月归档的历史 session 记录 | Agent 写 |
...
- **task_archive.json**：`task.json` 中 Done/Cancelled 任务超过 20 个时触发
- **recording_archive/YYYY-MM.md**：跨月首次启动 session 时触发，需人工确认
